 ...
 */
function validateEdgeCases(text, fieldName) {
 ...
 */
function validatePromptComplexity(prompt) {
    if (!prompt || typeof prompt !== 'string') {
        return { isValid: false, error: 'Prompt is required' };
    }
    const trimmedPrompt = prompt.trim();
...
class AIClient {
     ...
     */
    async executeMultimodalPrompt(textPrompt, imageFile, options) {
        const session = await this.createSession(options);
        try {
            ...
            throw new AIError('Failed to execute multimodal prompt', 'MULTIMODAL_EXECUTION_FAILED', error);
        }
    }
    /**
     * Execute a multimodal prompt with streaming response
     */
    async *executeMultimodalPromptStreaming(textPrompt, imageFile, options) {
        const session = await this.createSession(options);
        try {
            // Append multimodal content to session
            const messages = [
            ...
            throw new AIError('Failed to execute multimodal streaming prompt', 'MULTIMODAL_STREAMING_EXECUTION_FAILED', error);
        }
    }
    /**
...
if (typeof window !== 'undefined') {
    ...
    window.triggerModelDownload = triggerModelDownload;
...
class PromptExecutor {
    ...
    async executeRecipe(recipe, userInput, options = {}) {
        ...
        try {
            // Validate inputs
            if (!recipe || !userInput) {
                throw new AppError('Recipe and user input are required', 'INVALID_INPUTS', ErrorCategory.VALIDATION, ErrorSeverity.LOW, undefined, false, 'Please provide both a recipe and user input.');
            }
            // Check if AI client is available
            if (!(await this.aiClient.isAvailable())) {
                throw new AppError('AI client is not available. Please ensure Chrome AI API is enabled.', 'AI_CLIENT_NOT_AVAILABLE', ErrorCategory.AI_API, ErrorSeverity.CRITICAL, undefined, true, 'AI features are not available. Please check your Chrome settings.');
            }
            ...
            const response = await handleWithRetry(() => handleWithTimeout(() => this.aiClient.executePrompt(interpolatedPrompt, {
                temperature: options.temperature ?? 0.7,
                topK: options.topK ?? 40
            ...
            throw new AppError('Failed to execute recipe', 'EXECUTION_FAILED', ErrorCategory.AI_API, ErrorSeverity.HIGH, error, true, 'Failed to execute recipe. Please try again.');
        }
    }
    /**
     * Execute a recipe with multimodal input (text and image)
     */
    async executeRecipeMultimodal(recipe, userInput, imageFile, options = {}) {
        const startTime = Date.now();
        const sessionId = this.generateSessionId();
        try {
            // Validate inputs
            if (!recipe || (!userInput && !imageFile)) {
                throw new PromptExecutorError('Recipe and at least one input (text or image) are required', 'INVALID_INPUTS');
            }
            // Check if AI client is available
            if (!(await this.aiClient.isAvailable())) {
            ...
            const response = await this.aiClient.executeMultimodalPrompt(interpolatedPrompt, imageFile, {
                temperature: options.temperature ?? 0.7,
                topK: options.topK ?? 40
            });
            const executionTime = Date.now() - startTime;
            ...
            throw new PromptExecutorError('Failed to execute multimodal recipe', 'MULTIMODAL_EXECUTION_FAILED', error);
        }
    }
    /**
     * Execute a recipe with multimodal input and streaming response
     */
    async *executeRecipeMultimodalStreaming(recipe, userInput, imageFile, options = {}) {
        const startTime = Date.now();
        const sessionId = this.generateSessionId();
        try {
            // Validate inputs
            ...
            const stream = this.aiClient.executeMultimodalPromptStreaming(interpolatedPrompt, imageFile, {
                temperature: options.temperature ?? 0.7,
                topK: options.topK ?? 40
            });
            for await (const chunk of stream) {
            ...
            throw new PromptExecutorError('Failed to execute multimodal recipe with streaming', 'MULTIMODAL_STREAMING_EXECUTION_FAILED', error);
        }
    }
    /**
