...
import { AIClient, getAIClient } from './ai-client';
...
export class PromptEnhancer {
        ...
        try {
            ...
            try {
                ...
                const timeoutPromise = new Promise((_, reject) =>
                    setTimeout(() => reject(new Error('AI client initialization timeout')), 10000)
                );
     ...
     */
    private createEnhancementMetaPrompt(originalPrompt: string): string {
...
Enhanced prompt:`;
     ...
     */
    private cleanEnhancedPrompt(enhancedPrompt: string, originalPrompt: string): string {
     ...
     */
    private extractImprovements(original: string, enhanced: string): string[] {
     ...
     */
    private async enhancePromptDirectly(originalPrompt: string): Promise<string> {
        ...
        try {
            ...
            const timeoutPromise = new Promise<string>((_, reject) =>
                setTimeout(() => reject(new Error('Direct enhancement timeout')), 30000)
            );
     ...
     */
    public async testEnhancement(): Promise<boolean> {
        try {
